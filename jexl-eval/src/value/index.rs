...
impl<'a> fmt::Display for Type<'a> {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        match *self.0 {
            ...
            Value::DateTime(_) => formatter.write_str("datetime"),
            Value::Date(_) => formatter.write_str("date"),
            Value::Time(_) => formatter.write_str("time"),
            Value::Duration(_) => formatter.write_str("duration"),
            Value::SemVer(_) => formatter.write_str("semver"),
        }
    }
}
